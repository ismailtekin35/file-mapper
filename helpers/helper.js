...
module.exports = {
    mapToCityModel(value) {
        const retval = [...value.map((element, index, arr) => {
            return {
                "Color": "#FFF",
        ...
        const schema = {
            "Id": "4",
            "Title": "Locations",
            "Description": "All Turkey",
            "Type": "checkbox",
            "Selected": [
                {
                    "Color": "#FFF",
                    "Value": "reset",
                    "DisplayName": "All Turkey"
                }
            ],
            "FilterItem": []
        };
    ...
    mappingToTranslateJson() {
        ...
        Object.keys(jsonContetn).forEach(upperKey => {
            Object.keys(jsonContetn[upperKey]).forEach(async innerKey => {
                resultObj[upperKey][innerKey] = await translate.translate(jsonContetn[upperKey][innerKey], "tr");
    ...
    mapToCsv(fileNameParam) {
        ...
        for (let index = 0; index < sourceArr.length; index++) {
            ...
            if (index % 3 == 0) {
                ...
                console.log(sourceArr[index].replace("\r\n", ""), sourceArr[index + 1], [sourceArr[index + 2]])
    ...
    mapToCsv2(fileNameParam) {
        ...
        sourceArr.forEach((element, index) => {
            ...
            // if (mainCode[splitedElement[0]] != "") {
                ...
                mainCode[splitedElement[0]].push({ key: splitedElement[1], value: splitedElement[2] })
            // }
        });
        let retval = {};
        // mainCode.forEach(e => {
